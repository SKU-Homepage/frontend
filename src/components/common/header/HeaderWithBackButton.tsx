 ...
 * alt(이미지버튼 alt) : [optional]
 * msg(글자, 이미지버튼 이미지 없을시) : [optional]
 */

const HeaderWithBackButton = ({ src, msg, title, alt, onClick }: HeaderWithBackButtonProps) => {
  return (
    <header className="fixed top-0 z-20 flex h-[73px] w-full max-w-[600px] items-center justify-between bg-white px-[18px]">
      ...
      <HeaderImageButton src={src} msg={msg} alt={alt} onClick={onClick} />
    </header>
  );
};
